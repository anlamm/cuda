...
__global__ void matchKernel(char* sample, char* sign, int* match, double* score, char* qual,int n, int m, int sample_time, int sign_time, int sample_batch, int sign_batch) {
    ...
    if (sample_time*sample_batch+blockIdx.x < n && sign_time*sign_batch+threadIdx.x < m) {
        int ans = -1;
        int l = 0;
        for (int s = 0; s < 200000 - 500 + 1; ++s) {
            l = 0;
            bool end = false;
            while(true) {
                if (sign[threadIdx.x*10001 + l] == 0) {
                    ans = s;
                    break;
                }
                // || (s + l < 48000 && shared_sample_batch[s+l] == 0)
                else if (s + l >= 200000 || sample[blockIdx.x*200001+s+l] == 0) {
                    end = true;
                    break;
                }
                // (s + l < 48000 && (shared_sample_batch[s+l] == sign[threadIdx.x*10000 + l] || shared_sample_batch[s+l] == 'N')) || 
                else if (sign[threadIdx.x*10001 + l] == 'N' || sample[blockIdx.x*200001+s+l] == sign[threadIdx.x*10001 + l] || sample[blockIdx.x*200001+s+l] == 'N') {
                    ++l;
                }
                else {
                    break;
